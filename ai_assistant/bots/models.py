"""
Models for managing bots, conversations, knowledge bases,
chat messages, templates, and usage logs in the
...
class KnowledgeBase(models.Model):
    """
    Represents a file or manual input uploaded to a bot
    as a knowledge base.
    """
...
class KnowledgeChunk(models.Model):
    """
    A chunk of text extracted from a KnowledgeBase file,
    ...
    knowledge_file = models.ForeignKey(
        ...
        related_name="chunks",
        help_text=(
            "The parent knowledge file this chunk belongs to."
        ),
    )
    ...
    embedding = models.JSONField(
        ...
        blank=True,
        help_text=(
            "The vector embedding of this chunk "
            "for semantic search."
        ),
    )
    ...
    def __str__(self):
        return (
            f"Chunk of "
            f"{self.knowledge_file.file.name[:30]} "
...
class Bot(models.Model):
    """
    Represents a user-created AI bot with a defined
    personality and category.
    """
    ...
    description = models.TextField(
        blank=True,
        help_text="Short description of the bot.",
    )
    owner = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        help_text="User who owns the bot.",
    )
    created_at = models.DateTimeField(
        auto_now_add=True,
    ...
    category = models.CharField(
        ...
        default="general",
        help_text=(
            "Thematic category that shapes bot behavior."
        ),
    )
...
class Conversation(models.Model):
    ...
    public_id = models.UUIDField(
        ...
        db_index=True,
        help_text=(
            "Public identifier used by web, API, "
            "and mobile clients."
        ),
    )
    bot = models.ForeignKey(
        ...
        help_text="The bot used in this conversation.",
    )
...
class ChatMessage(models.Model):
    """
    Represents a single message exchanged between
...
class BotTemplate(models.Model):
    ...
    description = models.TextField(
        blank=True,
        help_text="Short description of the template.",
    )
    personality = models.TextField(
        default="I am a helpful and friendly assistant.",
        help_text=(
            "Default personality used by bots "
...
class BotUsageLog(models.Model):
    """
    Tracks how many tokens a user used with
    a specific bot.
    """
    ...
    tokens_used = models.IntegerField(
        help_text=(
            "Total number of tokens used during the session."
        ),
    )
    timestamp = models.DateTimeField(
        auto_now_add=True,
