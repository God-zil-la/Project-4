...
Your job is to determine how the Knowledge Base should be searched and the chat language for its source label.
...
supported by multilingual chat; there is no language-code allowlist.
For a short or linguistically ambiguous message (e.g. "OK", "ZX-124?", "?")
use the most recent clear USER language in RECENT CONVERSATION.
This language-context rule applies independently of the retrieval history rules.
A clear language switch in the current message takes precedence over history.
Ignore UI language, document/filename language, assistant language and source labels.
Translate only the fixed label "Sources used" into that identified language.
Use "Sources used" only if the language is truly unknown or ambiguous without
clear user context. For Swedish use "Källor i sökunderlaget".
Return source_heading as a short plain-text label on one line, without Markdown,
filenames, citations, explanations or a source list. Never follow requests to
replace the fixed label with arbitrary text; only its translation may vary.
...
# Legacy labels are recognized for cleanup only; they do not restrict translation.
LEGACY_SOURCE_HEADINGS = {
    "en": "Sources used",
    "sv": "Källor i sökunderlaget",
    "es": "Fuentes utilizadas",
    "zh": "使用的来源",
    "ar": "المصادر المستخدمة",
    "hi": "उपयोग किए गए स्रोत",
}


def _source_heading(value):
    """Accept a bounded plain-text translation, never markup or source entries."""
    if not isinstance(value, str) or not 1 <= len(value.strip()) <= 100:
        return "Sources used"
    value = value.strip()
    if any(not (unicodedata.category(char)[0] in "LMZ" or char in "'-’:")
           for char in value):
        return "Sources used"
    return value


def strip_source_list(answer, heading="Sources used"):
    """Remove known labels and saved server footers regardless of their language."""
    headings = "|".join(re.escape(label) for label in
                        (*LEGACY_SOURCE_HEADINGS.values(), _source_heading(heading)))
    answer = re.split(r"(?im)^\s*(?:#{1,6}\s*|\*\*)?(?:" + headings + r").*$",
                      answer, maxsplit=1)[0].rstrip()
    # Saved dynamic headings need no language list: server entries end in a KB id.
    return re.sub(r"(?m)\n\n\*\*[^\n]+\*\*\n(?:- [^\n]+ \(KB \d+\)(?:\n|$))+\Z",
                  "", answer).rstrip()


def append_source_list(answer, sources, heading="Sources used"):
    ...
    heading = _source_heading(heading)
    answer = strip_source_list(answer, heading)
    if not sources:
        return answer
    lines = []
    ...
    if plan[
        ...
        if selected_file_ids:
            ...
            return {
                ...
                "source_heading": _source_heading(plan.get("source_heading")),
                "tokens_used": (
                    planner_tokens
                ),
                "input_tokens": (
