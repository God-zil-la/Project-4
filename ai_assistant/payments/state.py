...
def has_existing_subscription(profile):
    return (
        bool(profile.stripe_subscription_id)
        and profile.stripe_subscription_status not in TERMINAL
    )
...
def validate_subscription(profile, subscription):
    ...
    if (
        subscription.get("customer") != profile.stripe_customer_id
        or (owner and str(owner) != str(profile.user_id))
        or subscription.get("livemode")
        is not settings.STRIPE_SECRET_KEY.startswith(
            ("sk_live_", "rk_live_")
        )
    ):
        raise ValueError(
            "Subscription ownership or mode mismatch"
        )


def clear_subscription(profile):
    # Google Play review account:
    # keep full Pro access even without a real Stripe subscription.
    if profile.user.username == "Mr.Husse":
    ...
    # A confirmed empty inventory must not leave a promised
    # future downgrade.
...
def reconcile_customer(profile):
    """
    Caller holds the profile lock.

    Verify the full customer inventory first.

    A missing/stale subscription link is cleared only after
    a successful complete listing. Provider failures and
    conflicting ownership always fail closed.
    """

    if not profile.stripe_customer_id:
        if profile.stripe_subscription_id:
            raise ValueError(
                "Missing billing customer"
            )
    ...
    if len(current) > 1:
        raise ValueError(
            "Multiple nonterminal subscriptions require support"
        )
